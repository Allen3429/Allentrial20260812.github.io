...
const requiredIds = ["connectionBadge","presenterHost","presenterHomeSlot","presenter","avatarLoading","landingPanel","trainingPanel","startBtn","avatarStage","interruptBtn","choiceArea","checkpointDialog","resultDialog","settingsDialog","fatalError","directAvatarState","directAvatarLog"];
...
for (const file of ["product-config.js","latency-bootstrap.js","campaign-data.js","perxona-sdk-guard.js","product-host.js","product.js","onboarding.js","onboarding.css","direct-avatar.js","direct-avatar.css","speed-pressure.js","speed-pressure.css","persona-preboot.js"]) assert.match(html, new RegExp(file.replace(".", "\\.")), `Active product asset is not loaded: ${file}`);
...
assert.doesNotMatch(onboarding, /scrollTrainingIntoView|queueTrainingScroll|window\.scrollTo/, "Starting training must not force-scroll the user's viewport");
assert.match(html, /onboarding\.js\?v=2\.1\.2/, "Onboarding cache version must expose the no-auto-scroll release");
...
console.log("ScamShield smoke test passed: direct Perxona interaction, resilient Ready lifecycle, fixed viewport, male persona, speed scoring, 12 rounds, and recovery paths are present.");
