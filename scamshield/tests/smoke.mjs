...
assert.match(html,/product-lite\.js\?v=1\.0\.2/,"Minimal controller must be active");
...
assert.match(lite,/function pickScene\(items\)\{ return items\[0\]; \}/,"Production scene selection must match the fast benchmark");
assert.match(lite,/function pickVoice\(items\)\{ return items\[0\]; \}/,"Production voice selection must match the fast benchmark");
...
console.log("ScamShield smoke test passed: one Perxona controller, benchmark-matched asset selection, verified fixed Avatar, visual interaction gate, 12 rounds, and speed scoring.");
